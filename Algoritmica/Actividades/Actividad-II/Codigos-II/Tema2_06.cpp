...
string infixToPostfix(string infixexpr)
{
    ...
    for (char token : infixexpr)
    {
        ...
        if (token == ' ')
            continue;
        else if (letsnums.find(token) <= letsnums.length()) //finds if the token is inside of letsnums and appends to the end of the container.
        ...
        {                                                                    //if the token is not inside of letsnums.
            while (!opStack.empty() && (prec[opStack.top()] >= prec[token])) //while the stack is not empty and the top item of the stack is on a higher level of PEMDAS than token.
            {
