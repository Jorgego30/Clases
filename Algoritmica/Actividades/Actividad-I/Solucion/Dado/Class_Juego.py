...
class Dado:
    ...
    def tirada(self):
        self.ultimo_resultado = random.randint(1, self.numeroCaras)
...
class Jugador:
    ...
    def __init__(self, nick, num_dados, num_caras):
...
class Juego:
    ...
    def __init__(self, num_turnos, num_dados, num_caras):
        """Crea un nuevo juego.
 
                :param num_turnos: El num. de turnos a jugar.
                :param num_dados: El num. de dados para todos.
                :param num_caras: El num. de caras para todos los dados.
        """
    ...
    def inserta_jugador(self, nick):
        self.jugadores += [Jugador(nick, self.num_dados, self.num_caras)]
    ...
    def __str__(self):
        ...
        posicion = 0
        max_pts = max(self.puntos_max)
        for i, pts in enumerate(self.puntos_max):
            toret += self.jugadores[i].nick + ": " + str(pts) + '\n'
            if pts == max_pts:
                posicion = i
                
        toret += "El ganador es " + self.jugadores[posicion].nick + ": " + str(pts) + '\n'
